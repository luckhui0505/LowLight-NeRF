...
def volumetric_rendering(rgb, density, darkness, t_vals, dirs, mode, 
    ...
    #coarse_dark = coarse_dark[:alpha.shape[1]]  # 切片 coarse_dark，使其与 alpha[..., :-1] 大小一致
    
    coarse_dark = torch.cat([coarse_dark, torch.zeros(alpha.shape[1] - coarse_dark.shape[0], device=coarse_dark.device)], dim=0)
    if fine_dark.shape[0] < alpha.shape[1]:
        # 填充使得 fine_dark 的长度与 alpha.shape[1] 一致
        fine_dark = torch.cat([fine_dark, torch.zeros(alpha.shape[1] - fine_dark.shape[0], device=fine_dark.device)], dim=0)
        # 如果 fine_dark 的大小大于 alpha 的第二维度，进行切割
    else:
        fine_dark = fine_dark[:alpha.shape[1]]
