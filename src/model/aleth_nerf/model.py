...
def low_light_enhancement(rgb, sigma, darkness):
    # 提高低光区域的亮度并保持其他区域的自然感
    dark_mask = (darkness < 0.5).float()  # 低光区域的掩码
    enhanced_rgb = rgb * (1 + dark_mask * 0.5)  # 对低光区域进行亮度增强
    return enhanced_rgb
...
class NeRFMLP(nn.Module):
    def __init__(
        ...
        k_ambient: float = 0.03  # 新增环境光强度参数c
    ):
        ...
        self.k_ambient = k_ambient  # 环境光强度

        #self.light_intensity = 0.05  # 主光源强度c
    ...
    def forward(self, x, condition):
        ...
        # 将环境光加到 RGB 输出中
        ambient_rgb = raw_rgb * self.k_ambient  # 环境光的影响
        raw_rgb += ambient_rgb  # 将环境光加到原始 RGB 上
        # 在此基础上增强低光区域
        enhanced_rgb = low_light_enhancement(raw_rgb, raw_density, raw_darkness)

        
        return  enhanced_rgb, raw_density, raw_darkness
...
class Aleth_NeRF(nn.Module):
    ...
    def forward(self, rays, randomized, white_bkgd, near, far, mode): # white_bkgd: False; near: None; far: None
        ...
        return ret
class LowLightContrastLoss(nn.Module):
    def __init__(self, contrast_factor=10.0):
        super(LowLightContrastLoss, self).__init__()
        self.contrast_factor = contrast_factor  # 控制对比度损失的强度

    def forward(self, pred_rgb, target_rgb):
        # 计算像素差异的平方作为对比损失
        contrast_loss = torch.mean((pred_rgb - target_rgb) ** 2)
        
        # 返回加权后的对比损失
        return contrast_loss * self.contrast_factor
...
class LitAleth_NeRF(LitModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 实例化 LowLightContrastLoss
        contrast_loss_fn = LowLightContrastLoss(contrast_factor=10)  # 可以调整 contrast_factor 的值
        contrast_loss = contrast_loss_fn(rgb_coarse_dark, target) + contrast_loss_fn(rgb_fine_dark, target)  # 计算低光照对比损失

        
        loss = loss1 + loss0  + 1e-3*loss_control + 1e-3*loss_structure + 1e-8*loss_cc + 1e-3*contrast_loss   # dark
        #loss = loss1 + loss0  + 1e-3*loss_control + 1e-3*loss_structure + 1e-8*loss_cc 
        ...
        self.log("train/contrast_loss", 1e-3*contrast_loss, on_step=True)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(  # 使用AdamW优化器
            ...
            lr=self.lr_init, 
            betas=(0.9, 0.99),  # 调整beta值
            weight_decay=1e-5)   # 添加权重衰减
    
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, 
            T_max=self.trainer.max_steps,  # 使用余弦退火
    ...
    def validation_epoch_end(self, outputs):
        ...
        with open('validation_results.txt', 'a') as file:
            file.write(f"Epoch {self.current_epoch}, PSNR: {psnr_mean.item()}, SSIM: {ssim_mean.item()}, LPIPS: {lpips_mean.item()}\n")
