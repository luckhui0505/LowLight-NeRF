...
class AdaptiveRaySampler(nn.Module):
    ...
    def compute_complexity(self, sigma_values):
        """根据密度场变化计算区域复杂度"""
        variance = torch.var(sigma_values, dim=-1, keepdim=True)
        normalized_variance = (variance - variance.min()) / (variance.max() - variance.min() + 1e-6)
        return normalized_variance

    def adaptive_sample(self, rays, sigma_prev=None):
        """
        自适应采样流程：
        1. 首轮使用基础采样数
        2. 根据上一轮密度场计算重要性
        ...
        if sigma_prev is None:
            # 初始均匀采样
            return helper.sample_along_rays(
                rays_o=rays["rays_o"],
                rays_d=rays["rays_d"],
                num_samples=self.base_samples,
                randomized=True
            )
        else:
            # 基于复杂度的自适应采样
            complexity = self.compute_complexity(sigma_prev)
            sample_counts = self.min_samples + (self.max_samples - self.min_samples) * complexity
            sample_counts = sample_counts.round().int().clamp(self.min_samples, self.max_samples)
            
            # 执行分层重要性采样
            return helper.stratified_importance_sample(
                rays,
                sample_counts,
                sigma_prev,
                randomized=True
...
class Aleth_NeRF(nn.Module):
    ...
    def compute_scene_complexity(self, rays, samples, t_vals):
        """
        计算场景复杂度，根据光照或其他启发式指标（如深度梯度）评估。
        """
        # 示例：使用深度差异或光照强度的变化作为场景复杂度的指标
        depth_gradients = torch.abs(samples[:, 1:] - samples[:, :-1])  # 简单的深度梯度启发式
        intensity_variations = torch.mean(depth_gradients, dim=-1)  # 平均光照强度变化

        complexity = intensity_variations.mean(dim=-1)  # 基于强度变化计算复杂度
        return complexity
    ...
    def forward(self, rays, randomized, white_bkgd, near, far, mode): # white_bkgd: False; near: None; far: None
        ...
        for i_level in range(self.num_levels):
            if i_level == 0:
                ...
                # 根据场景复杂度调整采样点数
                complexity = self.compute_scene_complexity(rays, samples, t_vals)
                self.num_coarse_samples = (self.num_coarse_samples * (1 + complexity)).round().int()  # 自适应粗采样
                self.num_coarse_samples = self.num_coarse_samples.float().mean() 
                self.num_coarse_samples = torch.round(self.num_coarse_samples)
                self.num_coarse_samples = int(self.num_coarse_samples.item())
                mlp = self.coarse_mlp
            else:
                ...
                # 根据场景复杂度调整采样点数
                complexity = self.compute_scene_complexity(rays, samples, t_vals)
                self.num_fine_samples = (self.num_fine_samples * (1 + complexity)).round().int()  # 自适应粗采样
                self.num_fine_samples = self.num_fine_samples.float().mean() 
                self.num_fine_samples= torch.round(self.num_fine_samples)
                self.num_fine_samples = int(self.num_fine_samples.item())
